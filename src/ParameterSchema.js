export const FALLBACK_PARAMETER_PAYLOAD = {
  ...
  parameters: {
    ...
    body_height: { label: 'Body height', min: 2.2, max: 4.2, default: 3.0, step: 0.05 },
    body_width: { label: 'Body width', min: 0.7, max: 1.45, default: 1.0, step: 0.02 },
    ...
    neck_length: { label: 'Neck length', min: 0.55, max: 1.35, default: 0.95, step: 0.02 },
    ...
    belly_color: { label: 'Belly color', type: 'color', default: '#8f9f59' },
    ...
    voxel_size: { label: 'Voxel size', min: 0.055, max: 0.18, default: 0.095, step: 0.005 },
...
const PARAMETER_RANGE_OVERRIDES = {
  ...
  body_height: { min: 2.2, max: 4.6, step: 0.05 },
  body_width: { min: 0.65, max: 1.55, step: 0.02 },
  torso_length: { min: 1.65, max: 3.25, step: 0.05 },
  pelvis_size: { min: 0.65, max: 1.4, step: 0.02 },
  neck_length: { min: 0.55, max: 1.65, step: 0.02 },
  ...
  voxel_size: { min: 0.035, max: 0.16, step: 0.005 },
  walk_speed: { min: 0.05, max: 1.0, default: 0.35, step: 0.05 },
...
export function randomizeParams(schema) {
  ...
  next.neck_length = Math.min(next.neck_length, next.dinosaur_length > 6.2 ? 1.2 : 1.0);
...
function randomSkinColor(key) {
  const palettes = {
    ...
    belly_color: ['#8fa35d', '#a09b68', '#8a8f67', '#9a8062', '#a4a078'],
  };
